...
void allocate_resources(void) {
    ...
    shmfd = shm_open(shmp, O_CREAT | O_EXCL | O_RDWR, PERMISSIONS,);
    if(shmfd == -1){
        error_exit("shm open");
    }
    ...
    shmp = mmap(NULL, SHM_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED smhfd, 0);
    if(shmp == MAP_FAILED){
        error_exit("mapping");
    }

    sem_request = sem_open(SEM_NAME_REQUEST, 1);    //  nicht RDWR sondern den wert
    sem_client = sem_open(SEM_NAME_CLIENT, 1);      //  nicht RDWR sondern den wert
    sem_response = sem_open(SEM_NAME_RESPONSE, 1);  //  nicht RDWR sondern den wert

    //fehlerbehandlung
    if(sem_client ==  SEM_FAILED   || sem_request == SEM_FAILED    || sem_response == SEM_FAILED){
        error_exit("semaphores");
    }
...
void process_password(const char *password, char *hash) {
    ...
    if(sem_wait(sem_client) == -1 && errno != EINTR){ //errno gehört hier nicht hin zwangsweise
        error_exit("sem client wait");
    }
    ...
    if(sem_post(sem_request) == -1){
        error_exit("sem post request");
    }

    //Server


    if(sem_wait(sem_response)==-1 && errno != EINTR){ //errno gehört hier nicht hin zwangsweise
        error_exit("sem wait response");
    }
