...
void parse_arguments(int argc, char *argv[], args_t *args) {
	...
	int quit = 0;
	while(!quit && (opt = getopt(argc, argv, "p:"))!=-1){
		switch(opt){
		case 'p':
			quit = 1;
			...
			break;	
		default: 
			usage("Incorrect Usage of password");
			exit(EXIT_FAILURE);
		}
...
void allocate_resources(void) {
    ...
    sem_client=sem_open(SEM_NAME_CLIENT, 1);
...
void process_password(const char *password, char *hash) {
    ...
    strncpy(shmp, password ,SHM_SIZE); // strcpy(shmp, password);
